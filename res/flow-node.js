...
module.exports = flow = function (temporaryFolder) {
	...
	$.post = function (req, callback) {
		...
		if (validation == 'valid') {
            ...
            //console.log('&.post - rename (%s => %s)', files[$.fileParameterName].path, chunkFilename);
			fs.rename(files[$.fileParameterName].path, chunkFilename, function (err) {

                //console.log('&.post - rename err: %s', err);

				// Do we have all the chunks?
				var currentTestChunk = 1;
				var numberOfChunks = Math.max(Math.floor(totalSize / (chunkSize * 1.0)), 1);
				var testChunkExists = function () {
                    //console.log('&.post - exists (%s)', getChunkFilename(currentTestChunk, identifier));
